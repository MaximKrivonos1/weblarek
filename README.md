...
Класс `PreviewCard` не хранит данные товара и не изменяет модель корзины напрямую. При нажатии на кнопку он создаёт событие `basket-toggle`, а решение о добавлении или удалении товара принимается в `main.ts` на основе данных моделей.
...
Экземпляр `BasketView` создаётся один раз, а при изменении корзины только ререндерится с новыми данными. Пересоздаются только карточки товаров внутри списка корзины.
...
`basket-toggle` - переключение состояния выбранного товара в корзине из карточки `PreviewCard`
...
###### Презентер
В данном проекте роль презентера выполняет файл `src/main.ts`. В нем находится основная orchestration-логика приложения: подписка на события, связывание моделей и представлений, открытие модальных окон и отправка заказа через API. Формы `OrderForm`, `ContactsForm` и представление `BasketView` создаются однократно и далее только ререндерятся при изменении данных моделей.
